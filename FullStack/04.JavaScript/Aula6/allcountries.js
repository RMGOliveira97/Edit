...
const selectCountry = document.querySelector("#worldCountries");
const containerInfos = document.querySelector(".product-details-js");

// Função para filtrar por todo o array qual o país com maior população por comparação
const maxPopCountry = countries.reduce(
  (acc, curr) =>
    acc.population < curr.population
      ? { name: curr.name.common, population: curr.population }
      : acc,
  { name: "", population: 0 }
);
// Função para aproveitar o código para a infos e passar como info da max population
displayCountry(maxPopCountry.name, ".max-pop-container");

const maxAreaCountry = countries.reduce(
  (acc, curr) =>
    acc.area < curr.area ? { name: curr.name.common, area: curr.area } : acc,
  { name: "", area: 0 }
);

displayCountry(maxAreaCountry.name, ".max-area-container");

// Selecionar o país + choose option
selectCountry.innerHTML = [
  `<option value="">Choose Country</option>`,
  ...countries.map(
    (country) =>
      `<option value="${country.name.common}">${country.name.official}</option>`
  ),
];

// Cada vez que se seleciona um país diferente muda as infos e quando não têm um país selecionado desaparece a box das infos
selectCountry.addEventListener("change", (event) => {
  event.target.value !== ""
    ? (containerInfos.style.visibility = "visible")
    : (containerInfos.style.visibility = "hidden");
  displayCountry(event.target.value, ".product-details-js");
});

// Função para mudar as infos
function displayCountry(selectCountry, parentClassContainer) {
  const country = countries.find(
    (country) => country.name.common === selectCountry
  );

  document.querySelector(parentClassContainer).innerHTML = `
  <div class="col-3">
  <img src="${country.flags.png}" alt="Flag" style="width: 100%" />
</div>
<div class="col-9">
  <div class="name-container">
    <span><h2>${country.name.official}</h2></span>
  </div>
  <div>
    <span
  ...
  <div>
    <span
      ><strong>Capital:</strong> <span>${country.capital}</spanclass=
    ></span>
  </div>
  <div>
    <span><strong>Area:</strong> </span><span>${country.area}</span>
  </div>
  <div>
    <span
      ><strong>Continent:</strong> <span>${country.region}</spanss=
    ></span>
  </div>
  <div>
    <span
      ><strong>Language:</strong> <span>${Object.values(
        country.languages
      )}</span
    ></span>
  </div>
...
document.querySelector(".clear-js").addEventListener("click", () => {
